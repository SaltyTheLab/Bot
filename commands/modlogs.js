...
export async function execute(interaction) {
    ...
    if (!usercheck) {
        return interaction.reply({
            embeds: [new EmbedBuilder({ color: 0x8d0b0b, author: { name: `❌ ${targetUser.tag} does not exist in Database!` } })]
        });
    } else if (!allLogs.length) {
        return interaction.reply({
            embeds: [new EmbedBuilder({ color: 0xf58931, author: { name: `⚠️ No modlogs found for ${targetUser.tag}.` } })],
        });
    }
    let currentIndex = 0;
            ...
            case 'del':
                ...
                if (allLogs.length < 1) {
                    await replyMessage.edit({
                        embeds: [new EmbedBuilder({ description: `All modlogs for ${targetUser} have been deleted.` })],
                        components: []
                    });
                    return;
                }
