import { SlashCommandBuilder, InteractionContextType, MessageFlags, ButtonStyle, LabelBuilder, TextInputStyle, ModalBuilder, } from "discord.js";
...
export async function execute(interaction) {
    ...
    if (applications[interaction.user.id] && applications[interaction.user.id].Activity) {
        interaction.reply({
            ...
            components: [{ type: 1, components: [{ type: 2, custom_id: 'next_modal_two', label: 'skip Part 1', style: ButtonStyle.Primary }] }],
        ...
        const questionOne = new LabelBuilder({
            ...
            component: {
                type: 3, max_values: 1, require: true, custom_id: 'age',
                options: [
        ...
        const questionTwo = new LabelBuilder({
            ...
            component: { type: 4, custom_id: 'experience', placeholder: 'Put your experience here or N/A', required: true, style: TextInputStyle.Paragraph, max_length: 300 }
        })
        const questionThree = new LabelBuilder({
            label: 'Have you been warned/muted/kicked/banned?',
            component: { type: 4, custom_id: 'punishments', placeholder: 'be honest and you do not need too much detail', required: true, style: TextInputStyle.Short, max_length: 100 }
        })
        const questionFour = new LabelBuilder({
            label: 'What is your timezone?',
            component: { type: 4, custom_id: 'timezone', required: true, style: TextInputStyle.Short, placeholder: 'If unknown, put your current time', max_length: 8 }
        })
        const questionFive = new LabelBuilder({
            label: `How active are you in ${guild.name}`,
            component: { type: 4, custom_id: 'activity', style: TextInputStyle.Short, required: true, max_length: 150 }
        })
