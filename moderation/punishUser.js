...
import { addPunishment, getActiveWarns, getUser } from '../Database/databasefunctions.js';
...
export default async function punishUser({ interaction = null, guild, target, moderatorUser, reason, channel, isAutomated = true, automodWarnWeight = 1, banflag = false, buttonflag = false, messageid = null
  ...
  let sentMessage = await channel.messages.fetch(messageid) ?? null;
  ...
  if (warnType === 'Warn') {
    const activeWarnings = await getActiveWarns(targetUser.id, guild.id);
    const totalWarnWeight = activeWarnings.length + currentWarnWeight;
    const punishment = getNextPunishment(totalWarnWeight);
    warnType = punishment.type;

    if (punishment.type === 'Mute') {
      durationMs = punishment.duration * unitMap[punishment.unit];
  ...
  else if (!buttonflag && !isAutomated && sentMessage == null)
    sentMessage = await interaction.reply({ embeds: [commandEmbed], fetchReply: true });


  const messageLink = sentMessage ?? `https://discord.com/channels/${guild.id}/${channel.id}/${sentMessage.id}`;
  ...
  try { await action } catch (err) { interaction.reply({ content: `I couldn't ${warnType} ${userTag}: ${err}` }) }
  if (!isAutomated)
