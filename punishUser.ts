...
import { ButtonStyle, ComponentType, InteractionResponseType, type APIEmbed, type APIUser, type APIChatInputApplicationCommandInteraction, type APIMessageComponentInteraction, } from 'discord.js'
 ...
 */
export default async function punishUser({ guildId, target, moderatorUser, reason, channelId, currentWarnWeight, interaction, banflag, kick }: PunishUserOptions) {
  ...
  const useravatar = await response({ method: "GET", endpoint: `https://cdn.discordapp.com/avatars/${target}/${avatar}` })
  ...
  if ((interaction!.data.options[0].name === 'Mute')) {
  ...
  const finalMessage: any = Response.json({ type: InteractionResponseType.ChannelMessageWithSource, data: cmdembed })
  ...
  const logEmbed: APIEmbed = {
    color: statusMap[warnType]!.color,
    author: { name: `${moderatorUser.username} ${statusMap[warnType]!.log}`, icon_url: `https://cdn.discordapp.com/avatars/${moderatorUser.id}/${moderatorUser.avatar}` },
  ...
  const dmchannel = await response({
    method: 'POST', endpoint: `/users/@me/channels`, body: {
      recipient_id: target
    }
  })
  ...
  logEmbed.footer!.text = dmResult === false ? 'User DMed 🚫' : 'User DMed ✅';
  switch (warnType) {
    case 'Ban':
      await guildconfigs.updateOne({ guildId }, { $set: { ban: target } });
      await response({ method: "PUT", endpoint: `/guilds/${target}/bans/${target}`, body: { delete_message_seconds: 604800 }, reason: `Ban Command: ${reason}` })
      break;
    case 'Mute':
      await response({ method: "PATCH", endpoint: `/guilds/${guildId}/members/${target}`, body: { communication_disabled_until: new Date(Date.now() + Math.min(durationMs, 2419200000)).toISOString() }, reason: reason })
      break;
    case 'Kick':
